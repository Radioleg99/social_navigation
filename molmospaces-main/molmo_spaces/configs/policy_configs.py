...
class MPPINavPolicyConfig(NavToObjPlannerPolicyConfig):
    """Configuration for MPPI-based mobile-base navigation."""

    policy_cls: type = None

    # Map construction
    planner_agent_radius: float = 0.22
    rollout_px_per_m: int = 200
    downscale: int = 5
    ...
    num_samples: int = 512
    num_iters: int = 4
    rollout_dt_s: float = 0.10
    command_dt_s: float = 0.20
    ...
    def model_post_init(self, __context) -> None:
        super().model_post_init(__context)
        if self.policy_cls is None:
            from molmo_spaces.policy.solvers.navigation.mppi_policy import MPPINavPolicy

            self.policy_cls = MPPINavPolicy
